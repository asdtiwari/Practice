...
/* An empty border gi es us a margin between teh edges of the panel and where the components are placed purely aesthetic */
        ...
        for (int i = 0; i < 256; i++)   {                   // Make tthe checkboxes,
    ...
    public void setUpMidi() {                                           // Theusual MIDI set-up stuff for
...
/* This is where it all happen! Where we trun checkbox static into MIDI enents and add them to the Track. 
    ...
    If that instrument is NOT supposed to p;ay on that beat, put in a zero. */
    public void buildTrackAndStart()    {
        int[] trackList = null;
        
        sequence.deleteTrack(track);    // get rid of the old track, amke a fresh one.
        track = sequence.createTrack();
        ...
        for (int i = 0; i < 16; i++) {      // do this for each of the 16 Rows (i.e. Bass, Congo, etc.)
            ...
            /* Set the 'key' that represents which instument this is (Bass, Hi-Hat, etc. 
            The insturments array hold the actual MIDI members for each instrument.)*/
            
            for (int j = 0; j < 16; j++) {  // Do this for each of the Beats for this row
                JCheckBox jc = (JCheckBox) checkBoxList.get(j + (16*i));
                if( jc.isSelected())    {   // Is the checlbox at this beat selected? If yes put
                    trackList[j] = key;     // the key value in this slot in the array (the slot that represents this beat).
                }   else    {                   // Otherwise, the insturment is NOT supposed to play at this beat, so set it to zero
                    trackList[j] = 0;
                }
            }   // close inner loop
            
            makeTracks(trackList);                                  // for this insturment, and for all 16 beats,
